...
if __name__ == "__main__":
    ...
    for log_count in [0, 3]:
        ...
        modes = ["res"] * n_members
        model_name = (
            "batches" + str(n_members) + "M_" + str(depths[0]) + "X" + str(widths[0])
        )
            ...
            for dataloader in [dataloader_train, dataloader_valid]:
                ...
                if dataloader == dataloader_valid:
                    ...
                    if epoch_loss < lowest_validation_loss:
                        temperature, _, _ = uqutils.perform_tempscale(
                            y_prim, y.to(DEVICE, torch.float)
                        )
        ...
        for corruption in range(0, 10):
            ...
            for x, x_classes, y in tqdm(iter(dataloader_test)):
                ...
                loss = loss_fn.forward(y_prim, y.to(DEVICE))
                temp_loss = loss_fn.forward(temp_scaled_y_prim, y.to(DEVICE))
                y = y.cpu()
                y_prim = y_prim.cpu()
                y_prims = y_prims.cpu()
                ...
                y_prim_idx = np.argmax(y_prim.data.numpy(), axis=1).astype(int)
                acc = np.mean((y_idx == y_prim_idx) * 1.0)
                accs.append(acc)
                conf = np.mean(np.max(y_prim.data.numpy(), axis=1))
                confs.append(conf)
        ...
        data.to_csv(
            "./data/experiments_better/" + model_name + "_temperature_results.csv",
            index=False,
        )
