...
if __name__ == '__main__':
    ...
    modes=["res"]*n_members
    model_name="temp"+str(n_members)+"M_"+str(depths[0])+"X"+str(widths[0])
...
# Scaling the pooled test probability with found tau*
    ...
    for corruption in range(0, 10):
        ...
        for x, x_classes, y in tqdm(iter(dataloader_test)):
            ...
            y = y.cpu()
            y_prims = y_prims.cpu()
            temp_scaled_y_prim=uqutils.prob_power_t(torch.mean(y_prims, 0).data.numpy(),temperature)
            loss = metrics.nll(temp_scaled_y_prim, y.data.numpy())

            losses.append(loss.item())
            model_losses.append(
                [metrics.nll(uqutils.prob_power_t(y_prim.data.numpy(),temperature), y.data.numpy()) for y_prim in y_prims]
            )
            y_idx = np.argmax(y.data.numpy(), axis=1).astype(int)
            y_prim_idx = np.argmax(temp_scaled_y_prim, axis=1).astype(int)
            acc = np.mean((y_idx == y_prim_idx) * 1.0)
            accs.append(acc)
            conf=np.mean(np.max(temp_scaled_y_prim,axis=1))
            confs.append(conf)
            y_for_ece = np.append(y_for_ece, y.data.numpy(), 0)
            y_prims_for_ece = np.append(
                y_prims_for_ece, temp_scaled_y_prim, 0
            )
