...
class f_s_SVGD:
    ...
    def phi(self, W, X, T, step, X_add=None):
        ...
        # Score function
        log_prob, pred = self.P.log_prob(
            W, X, T, return_pred=True, pred_idx=self.pred_idx
        )
        score_func = autograd.grad(
            log_prob.sum(), pred[self.pred_idx], retain_graph=True
        )[0]

        if X_add is not None:
            pred_add = (self.P.ensemble.forward(X_add, W)[self.pred_idx]).view(
                W.shape[0], -1
            )
        else:
        ...
        prior_pred = self.P.ensemble.forward(X, w_prior)[self.pred_idx].reshape(
            W.shape[0], -1
        )  # changed index here
        ...
        if self.noise:
            ...
            K_W_exp = torch.sqrt(
                2 * K_f.repeat(pred.shape[1], 1, 1) / (W.size(0) * lr)
            )  # K_XX.repeat(X.shape[1],1,1)*2
            langevin_noise = torch.randn_like(K_W_exp) * K_W_exp
            f_phi = (self.ann_schedule[step] * driv + num_t * grad_K) / W.size(
                0
            ) + langevin_noise.sum(2).T
        else:
