...
if __name__ == "__main__":
  ...
  # iterates through one training cycle for each member and one validation cycle
            ...
            data_iterators=[iter(dataloader) for dataloader in dataloaders_train]

            for _ in tqdm(range(len(data_iterators[0]))):
                for iterator in data_iterators:
                    x,x_classes,y=next(iterator)
                    y_prim, y_prims = model.forward(x.to(DEVICE), x_classes.to(DEVICE))
                    model_ensemble.current_model_train += 1
                    model_ensemble.current_model_train %= N_MEMBERS
            ...
            for x, x_classes, y in tqdm(iter(dataloader_valid)):
                y_prim, y_prims = model.forward(x.to(DEVICE), x_classes.to(DEVICE))
                loss = loss_fn.forward(y_prim, y.to(DEVICE))
