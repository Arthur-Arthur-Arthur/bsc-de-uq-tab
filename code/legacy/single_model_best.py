...
if __name__ == '__main__':
    ...
    for log_count in range(0,1):
        n_members=2**log_count
        depths=[8]*n_members #constant depth experiment
        widths=[int(1024/(n_members**2))]*n_members
        modes=["res"]*n_members
        model_name="single_best_"+str(depths[0])+"X"+str(widths[0])
        ...
        model.eval()
        data = pd.DataFrame(
            columns=["corruption", "ensemble_loss", "individual_losses", "accuracy","confidence", "std", "ece","time","epoch"]
        )
        data.loc[0,"time"] = best_model["epoch"]
